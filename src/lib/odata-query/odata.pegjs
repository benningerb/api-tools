...
single                      =   (   sign DIGIT "." DIGIT+ ( "e" / "E" ) sign DIGIT+
                                  / sign DIGIT* "." DIGIT+
                                  / sign DIGIT+
                                ) ("F" / "f")
                            /   nanInfinity ( "F" / "f" )?
...
//$maxpagesize
maxpagesize                 =   "$maxpagesize=" a:INT {return {'$maxpagesize': ~~a }; }
                            /   "$maxpagesize=" .* { return {"error": 'invalid $maxpagesize parameter'}; }
...
identifierPath              =   a:identifier b:identifierPathParts? {
                                    if (b == null) b = "";
                                    return a + (b || "");
                                }
selectList                  =   i:identifier list:("," WSP? l:selectList {return l;})? {
                                    if (list === "" || list == null) list = [];
                                    if (Array.isArray(list[0])) {
                                        list = list[0];
                                    }
